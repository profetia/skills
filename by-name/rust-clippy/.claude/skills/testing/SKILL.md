...
- When a test file for one lint also triggers other lints as side effects, use `#![expect(clippy::other_lint)]` at the top of the file to suppress them. Prefer `expect` over `allow` — `expect` validates that the other lint actually fires, catching cases where code changes accidentally make the suppression obsolete. Multiple lints can be combined: `#![expect(clippy::lint_a, clippy::lint_b)]`.
- When using `#![expect(clippy::other_lint)]` for a lint whose suggestion overlaps with the primary lint's suggestion, the `expect` attribute also prevents rustfix from applying the overlapping suggestion, avoiding "cannot replace slice" errors. In this case, ensure the expected lint still fires somewhere in the file after rustfix is applied (e.g., by adding a separate test case that triggers only the expected lint, not the primary one), so that the `.fixed` file also satisfies the `expect`.
